...
for i in range (3000):
	...
	#	from datetime import datetime
		...
		os.system("sed -i '$d' %s" % (kml_file))
		os.system("sed -i '$d' %s" % (kml_file))
		os.system("echo '<Placemark>' >> %s" % (kml_file))
		os.system("echo '<name>%s_%s</name>' >> %s" % (the_date3, the_time3,kml_file))
		os.system("echo '<description>' >> %s" % (kml_file))
		os.system("echo '<![CDATA[' >> %s" % (kml_file))
		os.system("echo '%s%s%s />' >> %s" % ('<img width="450" height="300" border="2" src="http://punkoryan.com/phidgetSBC2/',file_name,'"',kml_file))
