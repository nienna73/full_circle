...
for i in range (3000):
	...
	#	from datetime import datetime
		...
		#call(["exiftool", "-GPSLongitude='%i'", "-GPSLatitude='%i'", "%i" % (gps.getLongitude(), gps.getLatitude(), file_name)])
		#call(["exiftool", "-GPSLongitude='-106.646132'", "%s" % (file_name)]) #works!!!!!!!!!!!!!!!!!!!!
	#	curl -T pic.jpg -u phidgetSBC2@punkoryan.com:phidget2theLeft ftp://ftp.punkoryan.com
		#call(["curl", "-T", "pic.jpg", "-u", "phidgetSBC2@punkoryan.com:phidget2theLeft", "ftp://ftp.punkoryan.com"])
		#call(['curl', '-T', '%s', '-u', 'phidgetSBC2@punkoryan.com:phidget2theLeft', 'ftp://ftp.punkoryan.com' % (file_name)]) 
		import os
		
		os.system("exiftool -GPSLatitude=%s -GPSLongitude=%s %s" % (the_lat, the_long, file_name))
		import time
		time.sleep(10)
